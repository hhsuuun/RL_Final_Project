...
class Cell(IntEnum):
    ...
    TRAP = 2  # indicates a hole/trap; entering it ends the episode with a penalty
    CURRENT = 3  # indicates current cell of the agent
...
class Maze:
    """ A maze with walls. An agent is placed at the start cell and must find the exit cell by moving through the maze.
        ...
        penalty, returning to a cell the agent visited earlier gives a bigger penalty, running into a wall gives
        a large penalty, and stepping into a trap immediately ends the episode. The reward is collected when the
        agent reaches the exit. The game always reaches a terminal state; the agent either wins or loses.
    ...
    reward_exit = 20.0  # reward for reaching the exit cell
    ...
    def __init__(self, maze, start_cell=(0, 0), exit_cell=None):
        ...
        self.traps = [(col, row) for col in range(ncols) for row in range(nrows) if self.maze[row, col] == Cell.TRAP]
        ...
        if self.maze[self.__exit_cell[::-1]] in (Cell.OCCUPIED, Cell.TRAP):
    ...
    def reset(self, start_cell=(0, 0)):
        ...
        if self.maze[start_cell[::-1]] == Cell.TRAP:
            raise Exception("Error: start cell at {} is a trap".format(start_cell))
    ...
    def __execute(self, action):
        ...
        if not possible_actions:
            ...
            elif self.__current_cell in self.traps:
                reward = Maze.penalty_trap
    ...
    def __status(self):
        ...
        if self.__current_cell in self.traps:
            return Status.LOSE
    ...
    def render_q(self, model):
        ...
        if self.__render == Render.TRAINING:
            ...
            for trap in self.traps:
                self.__ax2.plot(*trap, "s", color="#d95f02", markersize=30)
                self.__ax2.text(*trap, "Trap", ha="center", va="center", color="white", fontsize=8)
            ...
            self.__draw_maze(self.__ax2)
            self.__ax2.get_figure().canvas.draw()

    def __draw_maze(self, ax):
        cmap = ListedColormap(["white", "black", "#f4a261", "#e76f51"])
        ax.imshow(self.maze, cmap=cmap, vmin=Cell.EMPTY, vmax=Cell.CURRENT)
